...
| 群聊触发 | 群里被 `@机器人` 或 `/` 开头命令触发；回复直接发出，不额外 `@` 发送者 | `/说明`、`/统计 关` 等斜杠命令无需被 @ |
...
- 执行成功后只回复一句确认（如 `✅ 已替你在群里艾特 田浩亢`），**不会重复你的原话**
- 网页版微信协议不支持真正的“提及高亮”，这里以文本 `@昵称` 形式发出，成员能在群里看到自己被点名

### 群聊触发与回复（无需被 @）

- 群里只要被 `@机器人` 或消息以 `/` 开头（如 `/说明`、`/艾特 张三 你好`、`/收款 50 李四 打车`）就会触发，
  普通闲聊不触发，避免打扰
- 机器人回复**不再 `@` 发送者**，直接在群里发出；只有执行“艾特某人”指令（如 `/艾特`、`/催收`）时才会带 `@`
