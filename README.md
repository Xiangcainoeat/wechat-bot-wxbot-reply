...
| 帮助 | 功能说明 | `@机器人` 或 `/说明` |
| 自然语言识别 | 自动识别意图并唤起提醒/天气/搜索/推送 | 直接说“提醒我10分钟后喝水”“上海天气” |
...
2. **AI 意图识别**：规则兜不住的自然语言，用同一个 AI 模型识别意图，再路由到已有功能
   - `提醒我10分钟后喝水` → 创建定时提醒
   - `上海天气` → 查询天气
   - `今天有什么足球比赛` → 联网搜索
   - `每天早上8点推送天气` → 设置每日天气推送（沿用城市确认流程）
   - 闲聊 → 仅已授权用户进入 AI 对话，未授权提示开通
3. **记账例外**：AI 识别为记账时**只回格式引导，绝不写入账本**，避免记错。记账必须用精确格式：
   `/记账 +8×4`（收入）、`/记账 -15×2`（支出）
