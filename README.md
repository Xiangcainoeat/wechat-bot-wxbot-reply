...
- 群聊：被 `@机器人` 或 `/` 开头命令触发（如 `/说明`、`/艾特 张三 你好`）；私聊：直接发消息触发
...
| 群内艾特 | 机器人替你在当前群里以 `@昵称` 形式给群成员发消息，**绝不私聊外发** | `/艾特 张三 你好`，或说“艾特张三说你好” |
| 群聊触发 | 群里被 `@机器人`、`/`、`\` 开头触发；回复后短时间免 `@` 继续聊 | `/说明`、`\帮我查一下xxx`；回复后直接接着聊 |
| 上下文统计开关 | 按会话隐藏/显示回复末尾的 token 用量统计（默认显示） | `/统计 关` / `/统计 开`，或说“隐藏上下文统计” |
| 收款登记 | 群内记账式登记别人欠款，可在群里艾特催收（网页版不支持真实转账） | `/收款 100 张三 买奶茶`、`/催收 张三`、`/欠款` |
| 防撤回 | 有人撤回消息时提示“撤回没用”并展示原内容（网页协议尽力而为） | 自动生效 |
...
同一个微信用户在**私聊**和**群聊**中是不同的会话，互不影响：

- **私聊**：每个用户一个独立会话（`用户ID`），上下文按人累计。
- **群聊**：同一群里**所有成员共享同一个会话**（`group:房间ID`），大家和机器人聊的内容在同一上下文里；
  你在私聊里让机器人记住的事（如称呼、偏好）不会带进群聊，群里发生的事也不会串到私聊。
- 压缩 / 新会话 / 智能体切换 / Codex 线程都按这个场景键隔离：群里执行 `压缩上下文` 只压缩群会话，
  私聊里的 `切换 codex` 不会改变群聊的智能体选择。
- 记账、提醒、每日推送等本地功能仍按用户独立，不受场景隔离影响。

### 群内艾特（替机器人发消息）

已授权用户可以让机器人在**当前群里**以 `@昵称 消息内容` 的形式替自己给群成员发消息：

- 命令：`/艾特 张三 你好`（`/at`、`/mention` 同义）
- 自然语言：`艾特张三说你好`、`帮我在群里艾特张三：晚上聚餐`、`提醒田浩亢 明天记得带电脑`、
  `把我艾特群里田浩亢提醒他`（“提醒谁/艾特谁/@谁”都会动态判断目标与内容）
- 目标是机器人自己（`@kindle` 触发前缀）、或“我/你/他/大家”这类指代时不当作艾特指令，交给 AI/提醒功能处理
- 只在群里使用：私聊里发 `/艾特` 会被拒绝，机器人**任何情况下都不会给别人发私聊消息**
- 机器人本体**永远不会成为艾特目标**：`@机器人` 只是触发前缀，`/艾特 kindle xxx` 也会被拒绝
- 执行成功后只回一句 **1~5 字随机确认**（如“已搞定”“妥了”“已办”），**不会复述你的原话**；
  复杂转达（如“帮我转告田浩亢明天带电脑”）由 AI 一次调用同时产出“艾特内容 + 简短确认”，本地连续发送，不重复调 API
- 网页版微信协议不支持真正的“提及高亮”，这里以文本 `@昵称` 形式发出，成员能在群里看到自己被点名

### 群聊触发与回复（无需被 @）

- 群里被 `@机器人`、或以 `/`、`\` 开头就会触发（如 `/说明`、`\帮我查一下今天天气`、`/收款 50 李四 打车`）
- **免 `@` 继续聊**：机器人刚回复过你的 15 分钟内，你在该群继续发消息不用再 `@`，像正常聊天一样（按人计时，不影响群里其他人）
- 反斜杠 `\` 开头 = 开启对话，正文直接交给 AI；斜杠 `/` 开头 = 命令
- 机器人回复**不再 `@` 发送者**，直接在群里发出；只有执行“艾特某人”指令时才会带 `@`
- 群里指挥艾特时**不再追问是哪个群**：一律在当前群发送

### 群聊长期记忆与称呼纠错

机器人在群里见过谁、谁有什么称呼，会写入群聊长期记忆（`group_memory.json`，按群隔离，不跨群共享）：

- 每次群里收到消息，自动记住“发送者 → 群内昵称”
- 可以直接告知称呼：`THk 群里这个人叫田浩亢`、`小明就是王小明`、`记住小明的真名是王小明`，
  机器人会写入长期记忆并回复确认；AI 从上下文推断出的称呼也会通过 `【记住称呼】别名=真名` 固化
- 艾特成功时把用户实际输入的称呼也记为别名，下次可直接命中（如记住“小明 = 王小明”）
- 执行 `/艾特`、`/收款`、`/催收`、`/打电话` 时自动做称呼解析：先精确匹配，再查群里称呼映射
  （别名 → 真实姓名），最后按包含关系/编辑距离做模糊纠错（错别字、多字少字、外号均可命中），
  命中后统一用群内规范昵称

### OpenClaw 会话管理

每个会话场景（私聊按用户、群聊按群共享）与 OpenClaw 的对话按独立会话保存，普通 AI 回复末尾会附**本次 API 调用**的 token 统计
（群聊里发消息时对机器人的 `@` 提及会被自动剥掉，不会原样发给 AI）
（如 `（上下文 6.8k / 128k，5.3%；输入 4.9k / 输出 2k；缓存命中 1792 / 未命中 3088）`）：
上下文总量 = 本次调用输入 + 输出，并给出输入/输出拆分与缓存命中/未命中数量，与后台（网关/线程文件）同源，
**不做计费**。上下文过大时可以压缩或另开新会话，旧会话不会丢失，可随时在后台查看或切回：

- `压缩上下文` / `/compact`：压缩当前会话（OpenClaw 原生压缩，命令本身不占用对话内容）
- `开启新的会话` / `/new` / `/新会话`：切换到全新上下文，旧会话保留
- 管理后台「会话与上下文」页：按用户/群聊查看历史会话与 transcript（群聊会话标「群聊」并按群名展示），
  可执行压缩、新会话、切换激活会话；所有 ID 均显示为短引用，不暴露完整微信 ID

**统计开关**：token 统计默认显示，可随时按会话隐藏/恢复，压缩回复、Codex 回复同样生效：

- 命令：`/统计 开`、`/统计 关`、`/统计 查看`
- 自然语言：说“隐藏上下文统计”“不要显示统计”= 隐藏；说“显示上下文统计”“开启统计”= 恢复

**长短回复由模型判断**：回复长度不再写死，由 OpenClaw/Codex 判断——闲聊、打招呼、表达情绪
（如“你好”“好的”“666”）用 1-5 句简短口语化回复；查询、分析、对比、总结、写内容时给出
完整详细的回答，可分点展开。
...
会话，回复末尾同样附**本次调用**的 token 统计：上下文总量 = 输入 + 输出、输入/输出拆分、
缓存命中/未命中，与线程文件 `last_token_usage` 同源，只显示当前这一轮，不再累加多轮总量。
其中「上下文」是**对话上下文**（单调累计，不含搜索/工具结果这类瞬时内容），若本轮调用
实际输入因搜索/工具而变大，会额外标注「本次调用输入」；后台「智能体」页可配置「Codex 上下文
窗口」，用于决定用量百分比的分母。
...
- **收款（记账式）**：网页版微信协议无法真实转账/收款，这里做**群内记账 + 催收**：`/收款 100 张三 买奶茶`
  登记“张三欠你 100 元”；`/欠款` 查看别人欠你多少；`/催收 张三` 会在群里以 `@张三` 形式提醒还钱
  （收款对象自动用群聊长期记忆解析成规范昵称）
- **防撤回**：有人撤回消息时，机器人若缓存过原内容会提示“撤回没用”并展示原内容；
  网页版协议不一定上报撤回事件，属**尽力而为**
- **打电话**：网页版微信协议不支持发起语音/视频通话，`/打电话 群昵称` 只会给出说明，**无法实现“接通即挂断”**
...
### 实现原理：Wechaty + 网页版协议（不是官方客户端）

该镜像本体是社区开源项目 `wechatbot-webhook`（基于 Wechaty 框架）的打包，**不是微信官方客户端**，也没有拆解官方客户端：

- **协议层**：用 Node.js 复刻了**网页版微信（`web.wechat.com`）的 HTTP 协议**（`wechat4u` / web puppet），不是注入、Hook 或改造官方 PC/手机客户端——官方客户端是闭源的，社区是在逆向分析网页版协议后重新实现了一个“客户端”
- **为什么扫码能登录**：网页版登录本来就是微信官方支持的通道。手机扫码 = 授权一次“网页版登录”，微信服务器下发登录态；容器把登录态存进 `loginSession.memory-card.json`（即挂载的 `/root/wxBot_session.json`），之后收发消息都带这个登录态走网页协议，所以重启不掉线、不用反复扫码
- **对微信服务器而言**：它只是一个“网页版微信”在收发消息，与浏览器里登录网页版微信没有本质区别
- **已知限制**：网页协议功能有限（部分消息类型收不到、不支持语音/视频通话等）；网页版存在风控，**登录几天会掉线、需要重新扫码**（`docker logs -f wxBotWebhook` 可观察）；属于非官方登录，高频使用有被限制甚至封号的风险，只适合低风险的个人/小范围用途
...
├── group_memory.json         # 群聊长期记忆：群名 + 成员称呼/别名（运行时生成）
├── collects.json             # 收款/欠款登记（运行时生成）
