...
3. **OpenClaw 问答**：天气、新闻、人物动态、资料查询、闲聊等其他问题不再进入本地搜索或旧 AI 路由，直接由 OpenClaw 按用户会话回答。
...
也可以在管理后台「AI 配置」页填写，用于单独测试旧兼容接口；微信里的 AI 问答和自然语言动作路由不使用这份配置，统一调用 OpenClaw。
...
机器人会把微信用户 ID 作为会话标识交给 OpenClaw，同一用户的后续提问可以延续上下文。服务器为微信入口使用独立的 OpenClaw 智能体，允许网页工具（搜索、读取网页）由 OpenClaw 在对话内部调用以核实实时信息，调用过程不会展示给用户；仍拒绝命令执行、文件读写、节点控制和 UI，提醒和每日推送是微信入口唯一的本地动作。现有 `main` 智能体和其他渠道不受影响。微信配置未启用 OpenClaw 时不会回退到旧 AI，而是返回统一不可用提示。
